...
export default {
  ...
  components: {
    ...
    parentFields: () => import("@/views/org/configurablePageManagement/parentFields/parentFields")
  },
  methods: {
    getComponentCode(val){
        this.curPopupsVisible = false
        this.curPopupsKey = this.curPopupsKey + 1
        this.$emit('changeParentFields',val)
      },
    close() {
        this.curPopupsVisible = false
