...
export default {
  ...
  components: {
    ...
    memberManage: () => import("@/components/org/memberManage/memberManage"),   dialogHeader: () => import("@/components/dialogHeader") //弹窗标题
  },
  filters: {
    statusFilter(status) {
      const statusMap = {
       ...
       this.listLoading = true;
        ...
        that.$requestAPI(paramD).then(response =>{
          if(response.result === '1' && response.rows!=''){
              if(page){
                  //查询完返回指定的page页数
                  that.nodeInfoList.pageIndex = response.pageIndexs;
                  that.nodeInfoList.pageSize = response.pages;
                  that.nodeInfoList.pageTotal = response.records;
              }
              that.tableList = response.rows;
      ...
      this.listLoading = true;
        ...
        that.$requestAPI(paramD).then(response =>{
          if(response.result === '1' && response.rows!=''){
              ...
              that.memberTableList = response.rows;
              that.listLoading = false;
            }
        })
    },
