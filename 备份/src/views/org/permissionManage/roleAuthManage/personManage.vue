...
export default {
  ...
  components: {
    OneTableTemplate,
    memberManage: () => import("./memberManage"),
  ...
  data() {
    return {
      ...
      tableCols: [
        ...
        { prop: "userName", label: this.$t('org.label.logInName'), width: null, align: "center" },// 用户名称
        { prop: "empName", label: this.$t('org.label.staffName'), width: null, align: "center" },// 员工姓名
      ...
      for(let i in arr){
        ...
        this.$requestAPI(paramD).then(response =>{
            if(response.result === '1'){
              ...
              this.$message({ message:"员工"+ arr[i].empName+'增加失败，' , type: "warning" });
            }
        })
      }
    }
