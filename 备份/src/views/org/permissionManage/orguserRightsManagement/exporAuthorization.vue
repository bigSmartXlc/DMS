    ...
    >
      ...
      <div style="text-align:right;padding-right: 10px;" v-if="showAuthorize">
        <el-button
          ...
          @click="authorize"
        >{{$t('org.label.authorization')/*授权*/}}</el-button>
      </div>
      <div
        class="filter-container filter-title"
        ref="resultTitleHeight"
      >{{$t('sys.content.gridTitle')/*查询结果*/}}</div>
...
export default {
  ...
  data() {
    return {
      ...
      showTitle:
        this.handleTitle == 0
          ? this.$t("org.label.userRightsView") /*用户权限查看*/
          : "用户授权" /*用户权限变更*/,
      //是否只查看有权限
      ...
      tableComponents: [
        {
          compKey: "compKey1",
          span: 12,
          labelName: this.$t("org.label.authorizedObject") /*授权对象*/,
          codeField: "userName",
          disabled: true,
  ...
  methods: {
    ...
    selectRoleEmpowerment(dataList) {
      ...
      for (let i in dataList) {
        if (
          dataList[i].menuName.indexOf(str) != -1 &&
          dataList[i].isSelected === "1"
        ) {
    ...
    authorize1(val) {
      ...
      this.$requestAPI(paramD).then(response => {
        if (response.result === "1") {
          this.$message({
            message:
              this.$t("org.label.userRightsChange") /*用户权限变更*/ +
              this.$t("sys.tips.esTip5") /*保存成功*/,
            type: "success",
            duration: 2000
          });
      ...
      if (data.length == 0) {
        ...
        for (var k in data) {
          obj = {
            ...
            objId: this.editData.userId,
            objName: this.editData.userName
          };
        ...
        <span class="custom-tree-node">
          ...
          <span>
            ...
            <input
              ...
              on-click={e => {
                let is = e.target.checked;
                document.getElementById("b" + data.menuId).checked = is
                if (is) {
                  if(!that.defaultCheckeDeys.includes(data.menuId)){
                    that.$refs.tree.setChecked(data.menuId, true);
                    if(data.children.length !== 0 && !data.children.children){
