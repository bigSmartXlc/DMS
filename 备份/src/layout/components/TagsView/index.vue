<template>
  ...
  >
    ...
    <div v-if="sysVer" style="position:absolute;bottom:0;height:20px;line-height: 20px;right: 80px;color: #999;pointer-events:none;">{{sysVer}}</div>
  </div>
</template>
...
export default {
  ...
  watch: {
    ...
    '$store.getters.sysConfig.SystemVerson': {
      handler: function() {
        this.setSysVer()
      },
    }
  ...
  methods: {
    setSysVer() {
      this.sysVer = this.$store.getters.sysConfig.SystemVerson ? '系统版本号：' + this.$store.getters.sysConfig.SystemVerson : ''
    },
    isActive(route) {
      ...
      if (vnode) {
        for (let i = 0; i < vnode.componentInstance.keys.length; i++) {
          ...
          if (path === view.path) {
            ...
            // 删除多余页面组件
            if (compIndex > -1) {
              that.deleteComponent(that.$parent.$children[compIndex], path);
            }
            break;
          }
        }
    ...
    // 根据AppMain删除组件
    deleteComponent(appMain, path) {
      if (appMain.$children) {
        appMain.$children.forEach((item, index) => {
          if (
            item.$vnode &&
            item.$vnode.data &&
            item.$vnode.data.key === path
          ) {
            appMain.$children.splice(index, 1);
            return;
          }
        });
      }
